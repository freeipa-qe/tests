...
cleanupRolesTests()
{
    ...
    rlRun "addPrivilegeToRole \"$privilegeName\" \"helpdesk\"" 
    roleName="helpdesk"
...
iparole_add_privilege_positive()
{
  ...
  rlPhaseStartTest "ipa-role-cli-1033 - add privilege to role --all" 
     ...
     expPriv="Privileges: User Administrators, Modify Group membership, Modify Users and Reset passwords"
...
iparole_remove_privilege()
{
  ...
  rlRun "kinitAs $ADMINID $ADMINPW"
   ...
   rlPhaseStartTest "ipa-role-cli-1040 - remove a user added privilege from role --all"
     ...
     rlRun "cat $TmpDir/iparole_removeprivilege.log" 0 "cat contents"
