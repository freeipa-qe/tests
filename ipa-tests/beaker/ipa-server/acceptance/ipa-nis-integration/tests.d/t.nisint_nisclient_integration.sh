...
nisint_nisclient_integration_setup_kerberos_for_auth()
{
	rlPhaseStartTest "nisint_nisclient_integration_setup_kerberos_for_auth: Setup Kerberos for NIS Client"
		...
		rlRun "sed -i '/EXAMPLE.COM = {/,/}/ s/^#//' /etc/krb5.conf"
		rlRun "sed -i 's/^# .example.com = EXAMPLE.COM/ .example.com = EXAMPLE.COM/' /etc/krb5.conf"
		rlRun "sed -i 's/^# example.com = EXAMPLE.COM/ example.com = EXAMPLE.COM/' /etc/krb5.conf"
		rlRun "sed -i 's/^# default_realm = EXAMPLE.COM/ default_realm = EXAMPLE.COM/' /etc/krb5.conf"
