...
nisint_nisclient()
{
	...
	##################################################################
	rhts-sync-block -s "nisclient_setup_start" $MASTER
	echo $FUNCNAME nisclient_setup_start
	nisint_nisclient_envsetup
	nisint_nisclient_check_nis_master
	rhts-sync-set -s "nisclient_setup_end" -m $NISCLIENT
	echo $FUNCNAME nisclient_setup_end

	##################################################################
	# nisclient_nisintegration: run NISCLIENT NIS Integration tests
	##################################################################
	rhts-sync-block -s "nisclient_nisintegration_start" $MASTER
	echo $FUNCNAME nisclient_nisintegration_start
	...
	rhts-sync-set -s "nisclient_nisintegration_end" -m $NISCLIENT
	echo $FUNCNAME nisclient_nisintegration_end

	##################################################################
	# nisclient_bzs: run NISCLIENT related BZ checks
	##################################################################
	rhts-sync-block -s "nisclient_bzs_start" $MASTER
	echo $FUNCNAME nisclient_bzs_start
	nisint_nisclient_netgroup_bzs
	nisint_nisclient_automount_bzs
	nisint_nisclient_services_bzs
	nisint_nisclient_other_bzs
	rhts-sync-set -s "nisclient_bzs_end" -m $NISCLIENT
	echo $FUNCNAME nisclient_bzs_end

	##################################################################
	# nisclient_migration: run NISCLIENT Migration tests
	##################################################################
	rhts-sync-block -s "nisclient_migration_start" $MASTER
	echo $FUNCNAME nisclient_migration_start
	nisint_nisclient_migrate_to_ipa
	nisint_nisclient_envcleanup
	rhts-sync-set -s "nisclient_migration_end" -m $NISCLIENT
	echo $FUNCNAME nisclient_migration_end
}
