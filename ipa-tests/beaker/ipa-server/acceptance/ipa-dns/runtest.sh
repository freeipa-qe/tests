...
# End neg add test cases
	...
	rlPhaseStartTest "ipa-dns-60 make sure that dig can find the record type cname"
		rlRun "dig cname.$zone CNAME | grep $cname" 0 "make sure dig can find the cname record"
	rlPhaseEnd

	rlPhaseStartTest "ipa-dns-61 delete record of type cname"
		rlRun "ipa dnsrecord-del $zone cname --cname-rec $cname" 0 "delete record type cname"
	rlPhaseEnd

	/etc/init.d/named restart

	rlPhaseStartTest "ipa-dns-62 make sure that IPA deleted record type cname"
		rlRun "ipa dnsrecord-find $zone cname" 1 "make sure ipa deleted record type cname"
	rlPhaseEnd

	sleep 5
	rlPhaseStartTest "ipa-dns-63 make sure that dig can not find the record type cname"
		rlRun "dig cname.$zone CNAME | grep $cname" 1 "make sure dig can not find the cname record"
	rlPhaseEnd

	# TXT Record
	rlPhaseStartTest "ipa-dns-64 add record of type txt"
		rlRun "ipa dnsrecord-add $zone txt --txt-rec $txt" 0 "add record type txt"
	rlPhaseEnd

	rlPhaseStartTest "ipa-dns-65 make sure that IPA saved record type txt"
		rlRun "ipa dnsrecord-find $zone txt | grep $txt" 0 "make sure ipa recieved record type txt"
	rlPhaseEnd

	rlPhaseStartTest "ipa-dns-66 make sure that dig can find the record type txt"
