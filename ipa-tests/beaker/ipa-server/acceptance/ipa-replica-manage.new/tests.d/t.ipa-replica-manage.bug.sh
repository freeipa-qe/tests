...
irm_bugcheck_831661()
{
	tmpout=$1
	logserver=$2
	rlLog "irm_bug_831661 - ipa-replica-manage re-initialize update failed due to named ldap timeout"
...
{
	tmpout=$1
	rlLog "irm_bugcheck_839638 - ipa-replica-manage allows disconnect of last connection for a single replica"
...
{
	tmpout=$1
	rlLog "irm_bugcheck_826677 - IPA cannot remove disconnected replica data to reconnect"
...
{
    tmpout=$1
    rlLog "irm_bugcheck_754539 - Connect after del using ipa-replica-manage fails"
...
{
    tmpout=$1
    rlLog "irm_bugcheck_823657 - ipa-replica-manage connect fails with GSSAPI error after delete if using previous kerberos ticket"
