...
irm_disconnect_negative()
{
	...
	"MASTER")
		...
		rlLog "0009 - disconnect - Deleted data is not replicated after Disconnect (master to replica)"
		rlRun "ipa host-del testhost1.$DOMAIN" 

		rlRun "rhts-sync-set -s '$FUNCNAME.$TESTORDER.1' -m $MASTER_IP"
		rlRun "rhts-sync-block -s '$FUNCNAME.$TESTORDER.2' $SLAVE1_IP $SLAVE2_IP"

		rlLog "0010 - disconnect - Added data is not replicated after Disconnect (replica to master)"
