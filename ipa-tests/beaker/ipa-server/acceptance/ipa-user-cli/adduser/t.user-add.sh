...
bugzillas()
{
    ...
    rlPhaseStartTest "trac558 setattr on RDN when user doesn't exist"
        expmsg="ipa: ERROR: test1: user not found"
        command="ipa user-mod --setattr=uid=test2 test1"
        rlRun "verifyErrorMsg \"$command\" \"$expmsg\"" 0 "Verify expected error message - alpha characters."
    rlPhaseEnd

    rlPhaseStartTest "bug643182 size limit too large"
    ...
    rlPhaseStartTest "bug569735 show user that doesn't exist"
        myuser=baduser
    ...
    rlPhaseStartTest "Bug801451 Logging in with GSSAPI should consult authentication authority policies (ssh pub keys tested with sssd)"
...
/usr/bin/ldapmodify -x -h $HOSTNAME -p 389 -D "cn=Directory Manager" -w $ADMINPW -c -f ./disableuser.ldif
    ...
    rlPhaseStartTest "bug805546 noprivate group specified gid number does not exist - default group non-posix"
    ...
    rlPhaseStartTest "bug805546 --noprivate gidnumber not specifiec - default group posix"
        ipa group-add --desc=test testgrp
        ggidnumber=`ipa group-find testgrp | grep "GID" | cut -d ":" -f 2`
        ggidnumber=`echo $ggidnumber`
    ...
    rlPhaseStartTest "bug805546 when adding a user with --noprivate option the gidNumber should be required"
    ...
    rlPhaseStartTest "bug721289 Process /usr/libexec/sssd/sssd_be was killed by signal 11 during auth when password for the user is not set."
    ...
    rlPhaseStartTest "bug835642 mail attribute not automatically populated"
