...
setup_ipa_http()
{
	rlPhaseStartTest "SETUP: IPA server - HTTP - Managed Host"
...
http_tests()
{
        rlPhaseStartTest "ipa-managedservices-http-001 Access HTTP service with valid credentials"
        ...
        rlPhaseStartTest "ipa-managedservices-http-002 Access HTTP service with out credentials"
	...
	rlPhaseStartTest "ipa-managedservices-http-003 Access HTTPS service with valid credentials"
                rlRun "kinitAs httpuser1 Secret123" 0 "kinit as user to get valid credentials"
        ...
        rlPhaseStartTest "ipa-managedservices-http-004 Access HTTPS service with out credentials"
                rlRun "kdestroy" 0 "destroy kerberos credentials"
                rlLog "Executing: curl -v --negotiate --cacert \"/etc/ipa/ca.crt\" -u: https://${HOSTNAME}:8443/ipatest/"
