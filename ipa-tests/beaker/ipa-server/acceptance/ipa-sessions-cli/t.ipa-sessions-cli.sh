...
sessionscli_basic()
{
	rlPhaseStartTest "001 - kinit as u1 and verify that the keyring gets created"
		kdestroy
	...
	rlPhaseStartTest "002 - clear local session keyring. Ensure that is appears cleared."
		rlRun "keyctl clear @s" 0 "Clear local session keyring"
		rlRun "keyctl list @s | grep ipa_session_cookie" 1 "Make sure that session keyring appears clear"
	rlPhaseEnd

	rlPhaseStartTest "003 - Ensure that ipa commands seem to not find valid session keys"
	...
	rlPhaseStartTest "004 - Ensure that ipa commands do not successfully find session keys"
	...
	rlPhaseStartTest "005 - Ensure that ipa commands seem to find valid session keys now that the keyring should be populated"
	...
	rlPhaseStartTest "006 - Ensure that ipa commands does successfully find session keys"
	...
	rlPhaseStartTest "007 - kinit as u2 and verify that the keyring gets created"
		kdestroy
	...
	rlPhaseStartTest "008 - Clear local session keyring. Ensure that is appears cleared."
		rlRun "keyctl clear @s" 0 "Clear local session keyring"
		rlRun "keyctl list @s | grep ipa_session_cookie" 1 "Make sure that session keyring appears clear"
	rlPhaseEnd

	rlPhaseStartTest "009 - Ensure that ipa commands seem to not find valid session keys"
	...
	rlPhaseStartTest "010 - Ensure that ipa commands do not successfully find session keys"
		rlRun "grep 'keyctl pipe' $outf" 1 "Ensure that keyctl pipe is not found"
		rlRun "grep 'keyctl pupdate' $outf" 1 "Ensure that keyctl update is not found"
	rlPhaseEnd

	rlPhaseStartTest "011 - Ensure that ipa commands seem to find valid session keys now that the keyring should be populated"
	...
	rlPhaseStartTest "012 - Ensure that ipa commands does successfully find session keys"
		rlRun "grep 'keyctl pipe' $outf" 0 "Ensure that keyctl pipe is found"
		rlRun "grep 'keyctl pupdate' $outf" 0 "Ensure that keyctl update is found"
	rlPhaseEnd

	rlPhaseStartTest "013 - Try it all again with host-find. First, clear local session keyring. Ensure that is appears cleared."
		rlRun "keyctl clear @s" 0 "Clear local session keyring"
		rlRun "keyctl list @s | grep ipa_session_cookie" 1 "Make sure that session keyring appears clear"
	rlPhaseEnd

	rlPhaseStartTest "013 - Ensure that ipa commands seem to not find valid session keys"
	...
	rlPhaseStartTest "014 - Ensure that ipa commands do not successfully find session keys"
		rlRun "grep 'keyctl pipe' $outf" 1 "Ensure that keyctl pipe is not found"
		rlRun "grep 'keyctl pupdate' $outf" 1 "Ensure that keyctl update is not found"
	rlPhaseEnd

	rlPhaseStartTest "015 - Ensure that ipa commands seem to find valid session keys now that the keyring should be populated"
	...
	rlPhaseStartTest "016 - Ensure that ipa commands does successfully find session keys"
		rlRun "grep 'keyctl pipe' $outf" 0 "Ensure that keyctl pipe is found"
		rlRun "grep 'keyctl pupdate' $outf" 0 "Ensure that keyctl update is found"
	rlPhaseEnd

	rlPhaseStartTest "017 - Kinit as admin. Ensure that the keyring gets populated."
		rlRun "keyctl clear @s" 0 "Clear local session keyring"
		rlRun "keyctl clear @u" 0 "Clear local user keyring"
		kdestroy
		KinitAsAdmin
	...
	rlPhaseStartTest "020 - Populate keyring with keys from two different users"
		kdestroy
		#keyctl purge user # Fedora only command: Purging keys to be certain that the user-find populates the keyring properly.
		rlRun "keyctl clear @s" 0 "Clear local session keyring"
		rlRun "keyctl clear @u" 0 "Clear local user keyring"
	...
	rlPhaseStartTest "021 - Populate keyring for u1. restart ipa_memcache. Ensure that the ipa session id changes"
		kdestroy
        local user="$u1"
		KinitAsUser $u1 $u1pass
	...
	rlPhaseStartTest "022 - Populate keyring for admin. restart ipa_memcache. Ensure that the ipa session id changes"
		kdestroy
        local user="admin"
		KinitAsAdmin
	...
	rlPhaseStartTest "023 - Create a valid keyring, then try issuing a command that will fallback to kerberos auth. Verify that the fallback happens"
		kdestroy
		#keyctl purge user # Fedora only command: Purging keys to be certain that the user-find populates the keyring properly.
		rlRun "keyctl clear @s" 0 "Clear local session keyring"
		rlRun "keyctl clear @u" 0 "Clear local user keyring"
	...
	rlPhaseStartTest "024 - If this test contains a slave, stop the server on the master."
	...
	rlPhaseStartTest "025 - get a key for u1 on the slave and make sure that it works."
		if [ -z $BEAKERSLAVE ]; then 
			rlLog "This test does not contain a slave. Not runnign multimaster tests."
			rlPass "test passed"
		else
	...
	rlPhaseStartTest "026 - Multi-host tests complete. Sync up and start master's IPA server again."
		if [ -z $BEAKERSLAVE ]; then 
			rlLog "This test does not contain a slave. Not runnign multimaster tests."
			rlPass "test passed"
		else
