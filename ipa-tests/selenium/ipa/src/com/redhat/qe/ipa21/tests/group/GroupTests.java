...
public class GroupTests extends TestScript{	
		...
		public void intialize() throws CloneNotSupportedException {		
			...
			selenium.open(SeleniumTestScript.ipaServerURL + "/ipa/ui/#identity=group&navigation=identity");
		...
		@Test (groups={"groupTests"}, dataProvider="getGroupTestObjects")	
		public void testGroupDelete(String testName, String groupName, String description) throws Exception {
			// wait for the User Groups page to be loaded
			...
			// verify group does exist
			com.redhat.qe.auto.testng.Assert.assertTrue(selenium.isTextPresent(groupName), groupName + " was added in previous test");
			
			//new test group can be added now
			GroupTasks.deleteGroup(selenium, groupName);			
			
			CommonTasks.waitForRefresh(selenium);
			
			//verify group was deleted successfully
			com.redhat.qe.auto.testng.Assert.assertFalse(selenium.isTextPresent(groupName), groupName + " delete successfully");
		}
