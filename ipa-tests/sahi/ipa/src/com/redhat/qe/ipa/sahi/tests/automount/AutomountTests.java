...
public class AutomountTests extends SahiTestScript{
	...
	/////////// add permission /////////////////////////
	@Test (groups={"addAutomount"}, dataProvider="addAutomount",
		description = "add new automount via 'Add' button")
	public void addAutomount_add(String automationLocation) throws Exception {  
		Assert.assertFalse(browser.link(automationLocation).exists(), "before add, automount location (" + automationLocation + ")should NOT exist in list");
		CommonHelper.addNewEntry(browser, automationLocation);  
		Assert.assertTrue(browser.link(automationLocation).exists(), "after add, automount location (" + automationLocation + ") should exist in list");
	}
	...
	@Test (groups={"deleteAutomount"}, dataProvider="leftOverLocations", dependsOnGroups="addAutomount",
			description="delete automount")
	public void deleteLeftOverPermission(String automationLocation) throws Exception { 
		Assert.assertTrue(browser.link(automationLocation).exists(), "before delete, autoumount location (" + automationLocation + ")should NOT exist in list");
		CommonHelper.deleteEntry(browser, automationLocation);  
		Assert.assertFalse(browser.link(automationLocation).exists(), "after delete, automount location (" + automationLocation + ") should exist in list");
	}
	...
	private static String[] testAutomountLocation = {"automountlocation000","automountlocation001","automountlocation002","automountlocation003","automountlocation004","automountlocation005","automountlocation006"};
	
	@DataProvider(name="addAutomount")
	public Object[][] getaddAutomount()
	{
		String[][] automountlocations = {{testAutomountLocation[0]} , {testAutomountLocation[1]} }; 
		return automountlocations;
	}

	@DataProvider(name="leftOverLocations")
	public Object[][] getLeftOverLocations()
	{
		return getaddAutomount();
	}
