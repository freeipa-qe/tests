...
tp3()
{
	...
	for s in $SERVERS; do
		if [ "$s" != "" ]; then
			...
			usernum=$minnum
			while [[ $usernum -lt $maxnum ]] ; do
				ssh root@$FULLHOSTNAME "ipa user-find --raw usr$usernum-x | /bin/grep dn: | /bin/grep uid=usr$usernum-x"
				if [ $? -ne 0 ]; then
...
tp4()
{
	...
	for s in $SERVERS; do
		if [ "$s" != "" ]; then
			...
			usernum=$minnum
			while [[ $usernum -lt $maxnum ]] ; do
				ssh root@$FULLHOSTNAME "ipa user-mod --last=testtesttestk$s usr$usernum-x"
				if [ $? -ne 0 ]; then
					echo "ERROR - search for usr$usernum-x on server $FULLHOSTNAME failed"
					tet_result FAIL
				fi
				eval_vars M1
				ssh root@$FULLHOSTNAME "ipa user-find --raw usr$usernum-x | /bin/grep sn | /bin/grep testtesttestk$s"
				if [ $? -ne 0 ]; then
...
tp5()
{
	...
	while [[ $usernum -lt $maxnum ]] ; do
		...
		ssh root@$FULLHOSTNAME "ipa user-del usr$usernum-x"
		if [ $? -ne 0 ]; then
...
tp6()
{
	...
	for s in $SERVERS; do
		if [ "$s" != "" ]; then
			...
			usernum=$minnum
			while [[ $usernum -lt $maxnum ]] ; do
				ssh root@$FULLHOSTNAME "ipa user-find usr$usernum-x"
				if [ $? -eq 0 ]; then
