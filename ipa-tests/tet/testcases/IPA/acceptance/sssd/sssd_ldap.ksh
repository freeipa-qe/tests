...
sssd_ldap_008()
{
   ...
   ####################################################################
        ...
        for c in $SSSD_CLIENTS ; do
                ...
                verifyCfg $c LDAP cache\-credentials TRUE
                if [ $? -ne 0 ] ; then
                        myresult=FAIL
                fi

                verifyCfg $c LDAP useFullyQualifiedNames TRUE
                if [ $? -ne 0 ] ; then
                        myresult=FAIL
                fi
