...
DATA=$TET_ROOT/../data
BASESUFFIX="o=mcom.com"

tp1() 
{
message "anonymous search uid=admin"

$LDAPSEARCH -p $LDAPport -h $LDAPhost -b "$BASESUFFIX" "uid=admin" > $RESULTS/acceptance_tp9.out
...
tp2()
{
message "turn off access to user uid=mward,ou=People,o=mcom.com"
os_run $LDAPMODIFY -p $LDAPport -h $LDAPhost -D "$ROOTDN" -w $ROOTDNPW -f $DATA/AS/$VER/acceptance/$CHARSET/acl.in
...
if [ $RC != 0 ]; then
	...
	W -f $DATA/AS/$VER/acceptance/$CHARSET/acl.in"
	tet_result FAIL
else
	tet_result PASS
fi
