...
# Test starts here 
$testuid="user_".$testid;
$testgid="grp_".$testid;
$testfulluid="uid=user_".$testid.",".$base;
...
sub run_test {
   ...
   # auto generated at 2008/5/26:10:37:52
	...
	$sel->open_ok("/ipa/ui/group/new");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->type_ok("form_cn", "$testdata->{'form_cn'}");
	$sel->type_ok("form_description", "$testdata->{'form_description'}");
	$sel->click_ok("submit");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->is_text_present_ok("$testdata->{'form_cn'} added!");
	$sel->is_text_present_ok("$testdata->{'form_cn'}");
	$sel->is_text_present_ok("$testdata->{'form_description'}");
} #admin_add_group
...
sub cleanup_data{
        ...
        IPAutil::ipa_deletegroup($ssh, $testdata->{'form_cn'});
        kdestroy();
}
