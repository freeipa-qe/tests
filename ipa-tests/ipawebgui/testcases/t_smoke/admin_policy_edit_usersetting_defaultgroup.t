...
our $testfullgid;
our $testdata;
our @datakeys=("form_ipadefaultprimarygroup");
...
sub run_test {
   # test case name (admin_policy_edit_usersetting_defaultgroup) from source (admin_policy_edit_usersetting_defaultgroup.pl)
   # auto generated at 2008/5/26:10:37:52
	...
	$sel->open_ok("/ipa/ui/ipapolicy/show"); 
	$sel->wait_for_page_to_load_ok("30000");
	$sel->click_ok("//input[\@value='Edit Policy']");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->type_ok("form_ipadefaultprimarygroup", "$testdata->{'form_ipadefaultprimarygroup'}");
	$sel->click_ok("submit");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->is_text_present_ok("IPA Policy updated");
	$sel->is_text_present_ok("Default User Group: $testdata->{'form_ipadefaultprimarygroup'}");
...
sub prepare_data{
        ...
        IPAutil::ipa_creategroup($ssh, $testdata->{'form_ipadefaultprimarygroup'}, $grpdesc);
}
...
sub cleanup_data{
        ...
        IPAutil::ipa_deletegroup($ssh, $testdata->{'form_ipadefaultprimarygroup'});
        kdestroy();
}
