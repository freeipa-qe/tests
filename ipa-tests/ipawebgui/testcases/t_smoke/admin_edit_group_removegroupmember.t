...
sub run_test {
   # test case name (admin_edit_group_removegroupmember) from source (admin_edit_group_removegroupmember.pl)
   # auto generated at 2008/5/26:10:37:52
	...
	$sel->wait_for_page_to_load_ok("30000");
	$sel->is_text_present_ok("$testgid_b [group]");
	$sel->click_ok("link=remove");
	$sel->click_ok("submit");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->is_text_present_ok("$testgid_a updated!");
} #admin_edit_group_removegroupmember
...
sub prepare_data{
        ...
        IPAutil::ipa_creategroup($ssh, $testgid_a, $grpdesc);
        IPAutil::ipa_creategroup($ssh, $testgid_b, $grpdesc);
        IPAutil::ipa_addgroup2group($ssh, $testgid_b, $testgid_a); # add testuid_b as member of testgid_a
}
...
sub cleanup_data{
        ...
        IPAutil::ipa_deletegroup($ssh, $testgid_a);
        IPAutil::ipa_deletegroup($ssh, $testgid_b);
        kdestroy();
}
