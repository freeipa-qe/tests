...
sub run_test {
   # test case name (admin_add_delegate_initial) from source (admin_add_delegate_initial.pl)
   # auto generated at 2008/5/16:10:54:41
	...
	$sel->click_ok("//a[\@onclick=\"selectGroup('source', 'cn=editor-initials,cn=groups,cn=accounts,dc=test,dc=com', 'editor-initials');                 return false;\"]");
	...
	$sel->click_ok("//a[\@onclick=\"selectGroup('dest', 'cn=users-initials,cn=groups,cn=accounts,dc=test,dc=com', 'users-initials');                 return false;\"]");
	$sel->click_ok("document.form.submit[2]");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->is_text_present_ok("delegate created");
	$sel->is_text_present_ok("initials 	editor-initials 	Initials 	users-initials");
} #admin_add_delegate_initial
