...
sub run_test {
   # test case name (admin_edituser_loginname) from source (admin_edituser_loginname.pl)
   # auto generated at 2008/5/26:10:37:52
	...
	$sel->type_ok("form_uid", "$testdata->{'form_uid'}");
	ok($sel->get_confirmation() =~ /^Are you sure you want to change the login name[\s\S]/ );
	$sel->click_ok("submit");
	$sel->wait_for_page_to_load_ok("30000");
	$sel->is_text_present_ok("Login: $testdata->{'form_uid'}");
